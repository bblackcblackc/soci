...
TEST_CASE_METHOD(common_tests, "Exception on not connected", "[core][exception]")
{
    ...
    CHECK_THROWS_AS(sql.begin(), soci_error &soci_error);
    CHECK_THROWS_AS(sql.commit(), soci_error &soci_error);
    CHECK_THROWS_AS(sql.rollback(), soci_error &soci_error);
    CHECK_THROWS_AS(sql.get_backend_name(), soci_error &soci_error);
    CHECK_THROWS_AS(sql.make_statement_backend(), soci_error &soci_error);
    CHECK_THROWS_AS(sql.make_rowid_backend(), soci_error &soci_error);
    CHECK_THROWS_AS(sql.make_blob_backend(), soci_error &soci_error);

    std::string s;
    long l;
    CHECK_THROWS_AS(sql.get_next_sequence_value(s, l), soci_error &soci_error);
    CHECK_THROWS_AS(sql.get_last_insert_id(s, l), soci_error &soci_error);
}
...
TEST_CASE_METHOD(common_tests, "Basic functionality", "[core][basics]")
{
    ...
    CHECK_THROWS_AS(sql << "drop table soci_test_nosuchtable", soci_error &soci_error);
...
TEST_CASE_METHOD(common_tests, "NULL expected exception", "[core][exception][null]")
{
    ...
    CHECK_THROWS_AS( std::for_each(rs.begin(), rs.end(), THelper()), soci_error &soci_error );
}
